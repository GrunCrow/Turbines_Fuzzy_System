...
#                                                       Splash Screen
splash_root = tk.Tk()
splash_root.geometry("320x320")
# Hide title bar
splash_root.overrideredirect(True)
image = ImageTk.PhotoImage(Image.open("images/resized_logo.png"))
...
class MainWindow:
    def get_inputs(self):
        input_salinity = self.textbox_input_salinity.get()
        input_temperature = self.textbox_input_temperature.get()
        input_currents = self.textbox_input_currents.get()
        input_laminar_height = self.textbox_input_laminar_height.get()
        input_viscosity = self.textbox_input_viscosity.get()
        input_density = self.textbox_input_density.get()
        input_depth = self.textbox_input_depth.get()
        input_placement_depth = self.textbox_input_placement_depth.get()
    ...
    def calculate(self):
        ...
        input_salinity, input_temperature, input_currents, input_laminar_height, input_viscosity, input_density, input_depth, input_placement_depth = self.get_inputs()

        if not (input_salinity or input_temperature or input_currents or input_laminar_height or input_viscosity or
                input_density or input_depth or input_placement_depth):
            input_salinity, input_temperature, input_currents, input_laminar_height, input_viscosity, input_density, \
            input_depth, input_placement_depth = load()

        if self.constraints(input_salinity, input_temperature, input_currents, input_laminar_height, input_viscosity, input_density, input_depth, input_placement_depth):
            ...
            res = tk.Label(self.frame_results, image=image, padx=20)
            res.pack()

            result = round(result, 3)
            result = str(result) + '%'
            result_cfg = font.Font(family='Nunito', size=12, weight="bold")
            label_result = tk.Label(self.frame_results, anchor="center", pady=10, text=result, fg=color_font,
                                    bg=color_bg_subframe)
            label_result['font'] = result_cfg
            label_result.pack()

    def open_popup(self, parameter, minimum_value, maximum_value):
    ...
    def constraints(self, input_salinity, input_temperature, input_currents, input_laminar_height, input_viscosity,
                    input_density, input_depth, input_placement_depth):
        if not (20 <= float(input_salinity) <= 50):
            self.open_popup(str_salinity, 20, 50)
        elif not (-20 <= float(input_temperature) <= 50):
            self.open_popup(str_temperature, -20, 50)
        elif not (0 <= float(input_currents) <= 300):
            self.open_popup(str_currents, 0, 300)
        # todo change Laminar Height values
        elif not (0 <= float(input_laminar_height) <= 100):
            self.open_popup(str_laminar_height, 0, 100)
        elif not (0.3 <= float(input_viscosity) <= 2):
            self.open_popup(str_viscosity, 0.3, 2)
        elif not (900 <= float(input_density) <= 1100):
            self.open_popup(str_density, 90, 1100)
        elif not (0 <= float(input_depth) <= 7000):
            self.open_popup(str_depth, 0, 7000)
        elif not (0 <= float(input_placement_depth) <= 150):
            self.open_popup(str_placement_depth, 0, 150)
        else:
            return True
    ...
    def clear_textbox(self):
        self.textbox_input_salinity.delete(0, 'end')
        self.textbox_input_temperature.delete(0, 'end')
        self.textbox_input_currents.delete(0, 'end')
        self.textbox_input_laminar_height.delete(0, 'end')
        self.textbox_input_viscosity.delete(0, 'end')
        self.textbox_input_density.delete(0, 'end')
        self.textbox_input_depth.delete(0, 'end')
        self.textbox_input_placement_depth.delete(0, 'end')

    def clear_results(self):
        not_empty = self.frame_results.winfo_children()
        if not_empty:
            for widgets in self.frame_results.winfo_children():
                widgets.destroy()
    ...
    def main_window(self):
        ...
        button_clear = tk.Button(frame, text=str_clear, fg=color_font, bg=color_main_bg, height=1, width=10, command=self.clear_all)  # , command=FuzzySystem.calculate()
        button_clear['font'] = button_cfg
        button_clear.grid(row=8, column=3, sticky="sw", pady=10)

        button_calculate = tk.Button(frame, text=str_calculate, fg=color_font, bg=color_main_bg, height=1, width=10, command=self.calculate())  # , command=FuzzySystem.calculate()
...
splash_root.after(5000, MainWindow.main_window)
tk.mainloop()
